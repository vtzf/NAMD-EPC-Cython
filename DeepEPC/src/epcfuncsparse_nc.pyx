...
cdef void epckq(
    ...
    double complex* epcq, int* tag, int[:,::1] R_list, int nspin, 
    int nmodes, int nm_num, int ncell, int nbands, int norbital, 
    int nbands2, MKL_INT norbnb
):
    cdef int i, j, k
    ...
    for i in range(nm_num):
        memset(dhvexpikR,0,nspin*norbnb*sizeof(double complex))
        for j in range(ncell):
            if tag[j] > 0:
                ...
                for k in range(nspin*norbnb):
                    dhvexpikR[k] += hamilveck[(i*ncell+j)*nspin*norbnb+k]*expikR
        cblas_zgemm3m(
            CblasRowMajor,CblasConjTrans,CblasNoTrans,
            nbands,nbands,nspin*norbital,&c1,&bandveckp[0,0],
...
def MPIepc(
    MPI.Comm comm,
    int nspin, int nmodes, int nm_num, int nm_min, int norbital, int nbands, 
    int ncell, int knum, int[::1] nq, int[:,::1] R_list, int nkpath, 
    double[:,::1] kpath, int[:,::1] key_num, int[:,::1] key_num_s, 
    int[:,::1] key_info, int[::1] key_info_s, double complex[:,:,::1] dhamil, 
    ...
    dhexpikR = <double complex*>malloc(sizeof(double complex)*nspin2*matlen)

    dhvexpikR = <double complex*>malloc(sizeof(double complex)*nspin*norbnb)
    vdhvexpikR = <double complex*>malloc(sizeof(double complex)*nbands2)
    len_hv = sizeof(double complex)*nm_num*ncell*norbnb_l*nspin
    ...
    for h in range(knum_p):
        ...
        if (kt != k1):
            ...
            memset(hamilveck,0,len_hv)
            hamilveck_t = <MKL_Complex16*>(&hamilveck[0])
            for i in range(nm_num):
                for j in range(ncell):
                    if tag[j] > 0:
                        dhlen = key_num_s[j,0]
                        memset(dhexpikR,0,nspin2*dhlen*sizeof(double complex))
                                ...
                                if n == norb2:
                                    for spin in range(nspin2):
                                        for l in range(norb2):
                                            dhexpikR[spin*norb2+l] \
                                            += dhamil[i,spin,m+l]*expikR
                                else:
                                    for spin in range(nspin2):
                                        for l in range(n):
                                            dhexpikR[spin*dhlen+key_info[m+l,1]] \
                                            += dhamil[i,spin,m+l]*expikR
                        if tag[j] == 1:
                            for spin in range(nspin2):
                                s_r = <int>(spin/nspin)
                            ...
                            for spin in range(nspin2):
                                ...
                                cblas_zgemm3m(
                                    ...
                                    nbands,norbital,&c1,&dhexpikR[spin*norb2],norbital,
                                    &bandveck[kidx,s_r*norbital,0],nbands,&c1,
                                    &hamilveck_t[s_c*norbnb],nbands
                                )
                    hamilveck_t += nspin*norbnb
            kt = k1

        epckq(
            kpx,kpy,kpz,hamilveck,bandveck[kpidx],phvecval[k2],
            dhvexpikR,vdhvexpikR,&epc_t[h,0],tag,R_list,nspin,
    ...
    for h in range(knum_p):
                    ...
                    if tag[j] == 1:
                        ...
                        for spin in range(nspin2):
                            ...
                            cblas_zgemm(
                                CblasRowMajor,CblasNoTrans,CblasNoTrans,norbital,
                                nbands,norbital,&c1,&dhexpikR[spin*norb2],norbital,
                                &bandveck[k1,s_r*norbital,0],nbands,&c1,
                                &hamilveck_t[s_c*norbnb],nbands
                            )
                hamilveck_t += nspin*norbnb
        ...
        for i in range(nqpath):
            ...
            epckq(
                ...
                nspin,nmodes,nm_num,ncell,nbands,norbital,nbands2,norbnb
            )
...
cdef void epckq_p(
    ...
    int[:,::1] R_list, int nspin, int nmodes, int nm_num, int ncell, int norbital
):
    cdef int i, j, k
    cdef double RKpx, RKpy, RKpz, RKp
    cdef double complex pi2j = M_PI*2j
    ...
    for i in range(nm_num):
        memset(dhvexpikR,0,nspin*norbital*sizeof(double complex))
        for j in range(ncell):
            if tag[j] > 0:
                RKpx = R_list[j,0]*Kpx
                RKpy = R_list[j,1]*Kpy
                ...
                RKp = RKpx + RKpy + RKpz
                expikR = cexp(-1.0*pi2j*RKp)
                for k in range(nspin*norbital):
                    dhvexpikR[k] += hamilveck[(i*ncell+j)*nspin*norbital+k]*expikR
        cblas_zdotc_sub(
            nspin*norbital,bandveckp,1,dhvexpikR,1,&vdhvexpikR
        )
        for j in range(nmodes):
...
def MPIepc_p(
    MPI.Comm comm,
    int nspin, int nmodes, int nm_num, int nm_min, int norbital, int ncell, 
    int knum, int[::1] nq, int[:,::1] R_list, int[:,::1] key_num, 
    int[:,::1] key_num_s, int[:,::1] key_info, int[::1] key_info_s, 
    double complex[:,:,::1] dhamil, double complex[:,::1] bandveck, 
    ...
    dhvexpikR = <double complex*>malloc(sizeof(double complex)*nspin*norbital)
    len_hv = sizeof(double complex)*nm_num*ncell*norbital_l*nspin
    hamilveck = <double complex*>malloc(len_hv)
                        ...
                        if tag[j] == 1:
                            ...
                            for spin in range(nspin2):
                                ...
                                cblas_zgemv(
                                    ...
                                    norbital,&c1,&dhexpikR[spin*norb2],norbital,
                                    &bandveck[idx1,s_r*norbital],1,&c1,
                                    &hamilveck_t[s_c*norbital],1
                                )
                    hamilveck_t += nspin*norbital
            kt = idx1

        epckq_p(
            kpx,kpy,kpz,hamilveck,&bandveck[idx2,0],phvecval[qidx],
            dhvexpikR,&epc_t[h,0],tag,R_list,nspin,nmodes,nm_num,ncell,norbital
        )
