...
cdef herr_t readh5engine_key1(
    ...
    cdef herr_t status
    cdef hid_t data_id
    cdef int i, j, iR, jR, Rij, atomi, atomj
    global key_buf
...
cdef void readscfout1(
    ...
    cdef int TCpyCell
    cdef int i, j ,k, ct_AN, h_AN, Gh_AN, atomnum
    cdef int iR, jR, Rij, TNO1, TNO2, offset
    cdef int* atv_ijk
    cdef int* FNAN
    cdef int** natn
    cdef int** ncn
...
cdef void readh5_p1(
    ...
    double factor, double* data_buf, double* hamil_buf, double[::1] dhamil, bint LADD
):
    cdef hid_t f, data_id
    cdef herr_t status
    cdef int h, i, j, key_min, key_max, offset
...
#            for i in range(nnodes):
#                imodes_num_gather[i] = imodes_num_shm[i]*ncell2
#            for i in range(nnodes+1):
#                imodes_gather[i] = imodes_shm[i]*ncell2
#            mpi.MPI_Allgatherv(
#                mpi.MPI_IN_PLACE,0,mpi.MPI_DATATYPE_NULL,&dhamil[0,0],
