...
cdef herr_t readh5engine_key1(
    ...
    cdef herr_t status
    cdef hid_t data_id
    cdef int i, j, atomi, atomj, TNOi, TNOj
    global key_num_p
...
cdef void readscfout(
    ...
    double factor_h, double factor_o, double factor_dr, int Ispin
):
    cdef FILE * fp
    cdef int i_vec[6]
    cdef int TCpyCell
    cdef int i, j ,k, xyz, ct_AN, h_AN, Gh_AN, \
             atomnum, factor, offset, TNO1, TNO2, spin
    cdef int* atv_ijk
    cdef int* FNAN
    cdef int** natn
    cdef int** ncn
...
def olp_inv(
    ...
    # ORIGINAL code used mkl_sparse_d_create_csr + mkl_sparse_optimize +
    # mkl_sparse_d_mm + mkl_sparse_destroy. This triggers MKL's AVX-512 path
    # (mkl_sparse_d_do_destroy_i4_avx512) which has a bug causing SIGSEGV on
    # this MKL version. Replaced with: expand CSR to dense + cblas_dgemm.
    dr_dense = <double*>malloc(s_d*Np_mkl*N_mkl)
    for i in range(3):
        # expand sparse CSR dr[i] -> dense dr_dense[Np_mkl x N_mkl]
        memset(dr_dense,0,s_d*Np_mkl*N_mkl)
        for j in range(Np_mkl):
            for k in range(dr_csr_ridx[0,j],dr_csr_ridx[1,j]):
                dr_dense[j*N_mkl+dr_csr_cidx[drp_min+k]] = dr[i,drp_min+k]
        # dot(dr,S^-1*H) [Np,N]*[N,Nproc_len]
        cblas_dgemm(
            101,111,111,Np_mkl,lldSp,N_mkl,
            1.0,dr_dense,N_mkl,olpinvham,lldSp,
            0.0,matbuf,lldSp
        )
