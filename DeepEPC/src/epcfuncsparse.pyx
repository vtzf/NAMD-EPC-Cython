...
def MPIepc(
    MPI.Comm comm,
    int nmodes, int nm_num, int nm_min, int norbital, int nbands, 
...
def MPIepc_q(
    MPI.Comm comm,
    int nmodes, int nm_num, int nm_min, int norbital, int nbands, int ncell, int knum,
    int[::1] nq, int[:,::1] R_list, int nqpath, double[:,::1] qpath, 
    int[:,::1] key_num, int[:,::1] key_num_s, int[:,::1] key_info,
    int[::1] key_info_s, double[:,::1] dhamil,
    double complex[:,:,::1] bandveck, double complex[:,:,::1] phvecval,
    int[::1] kproc, int[::1] kproc_num, double complex[:,::1] epc_t
):
    cdef mpi.MPI_Comm c_comm = comm.ob_mpi
...
def MPIepc_p(
    MPI.Comm comm,
    int nmodes, int nm_num, int nm_min, int norbital, int ncell, 
