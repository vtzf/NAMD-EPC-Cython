...
def phonon(inDir,phononDir,infile,q_list):
    comm = MPI.COMM_WORLD
    myid = comm.Get_rank()
    ...
    # read data
    if shm_id == 0:
        fc[:] = np.load(inDir+phononDir+'fc_avg.npy')
...
if shm_id == 0:
    qidx = np.zeros((qnum,3),dtype=int)
    idx = np.arange(qnum)
    qidx[:,2] = idx%nq[2]; qidx_xy = idx//nq[2]
    qidx[:,1] = qidx_xy%nq[1]; qidx[:,0] = qidx_xy//nq[1]
    q_list[:] = qidx/nq

#    q_list[0:100] = np.linspace([0,0,0],[0,1/2,0],100,endpoint=False)
#    q_list[100:200] = np.linspace([0,1/2,0],[1/2,1/2,0],100,endpoint=False)
#    q_list[200:300] = np.linspace([1/2,1/2,0],[0,0,0],100,endpoint=False)
#    q_list[300:400] = np.linspace([0,0,0],[0,0,1/2],100,endpoint=False)
#    q_list[400:500] = np.linspace([0,0,1/2],[0,1/2,1/2],100,endpoint=False)
#    q_list[500:600] = np.linspace([0,1/2,1/2],[1/2,1/2,1/2],100,endpoint=False)
#    q_list[600:700] = np.linspace([1/2,1/2,1/2],[0,0,1/2],100,endpoint=False)
shm_comm.Barrier()
phonon(inDir,phononDir,infile,q_list)
