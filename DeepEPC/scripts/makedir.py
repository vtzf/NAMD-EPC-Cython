    ...
    for i in catom:
        for j,k in zip(['x','y','z'],[0,1,2]):
            ...
            for l in range(xyz.shape[0]):
                xyzw = info1[l].split()
                f_h.write('%3s%4s%18.12f%18.12f%18.12f%5s%5s%5s\n'\
                         %(xyzw[0],xyzw[1],\
                           xyz_tmp[l,0],xyz_tmp[l,1],xyz_tmp[l,2],\
                           xyzw[5],xyzw[6],xyzw[7]))
            for l in info2:
                f_h.write(l)
            f_h.close()
...
def write_ucell():
    ...
    atom_u = [j for j in atomidx]
    charge_u = [j for j in charge]
    ...
    for i in range(atom.shape[0]):
        f.write('%3d  %2s'%(i+1,atom_u[i]))
        for j in range(3):
            f.write('%18.12f'%(atom[i,j]))
        f.write('  %3.1f  %3.1f  off\n'%(charge_u[i],charge_u[i]))
    f.write("Atoms.SpeciesAndCoordinates>\n")
