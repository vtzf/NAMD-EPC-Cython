...
//        CompTime[myid][2] += truncation(MD_iter,1);
    ...
    if (myid==Host_ID){
      for (i=0; i<argc; i++){
        if ( strcmp(argv[i],"-calcOLPmo")==0 )
          IsDolp = 1;
      }
    }
    MPI_Bcast(&IsDolp, 1, MPI_INT, Host_ID, MPI_COMM_WORLD1);
    
    if (IsDolp==1)
    {
      time2 = Set_Orbitals_Grid(0);
      const double Bohr2Ang = 0.529177249;
      int direction, Mc_AN, Gc_AN, h_AN, Gh_AN, tno0, tno1, Cwan, Hwan;
      double *****OLPmo;
      ...
      Calc_OLPmo(OLPmo);
      output_O_nm(OLPmo[0], "output/overlaps_dx", 0, 1.0/Bohr2Ang);
      output_O_nm(OLPmo[1], "output/overlaps_dy", 0, 1.0/Bohr2Ang);
      output_O_nm(OLPmo[2], "output/overlaps_dz", 0, 1.0/Bohr2Ang);
      if (myid==Host_ID)
          printf("\n Finish calc. OLPmo\n");
      for (direction=0; direction<3; direction++){
...
void output_O_nm(double ****O, const char *output_name, int ParallelHDF5, double coefficient)
{
	...
	if (numprocs==1)
	    sprintf(output_name_id, "%s.h5", output_name);
        else
