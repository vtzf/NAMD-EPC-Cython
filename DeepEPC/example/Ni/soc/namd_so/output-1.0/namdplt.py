...
#    kplabels = [r'$[\frac{1}{3},\frac{1}{3}-\frac{1}{60}]$',r'$[\frac{1}{3},\frac{1}{3}]$',r'$[\frac{1}{3},\frac{1}{3}+\frac{1}{60}]$']
#    kpath = np.array([ # for TDBAND.png
#        [0.33333, 0.31666, 0.00000],
#        [0.33333, 0.33333, 0.00000],
#        [0.33333, 0.35000, 0.00000]
#        ])

    kplabels = ['$\Gamma$','M','K','$\Gamma$']
    kpath = np.array(\
    ...
    qplabels = kplabels
#    qplabels = ['[0.0,0.0]','[0.0,0.5]','[0.0,1.0]']
#    qpath = np.array([
#        [0.00000, 0.00000, 0.00000],
...
#    print(k_index)
    ...
    if (l_read_shp):
        ...
        shp = np.zeros((NSW,nbasis+2),dtype=np.float32)
        for i in range(len(fileshp_e)):
            shp[:,1] += np.load(fileshp_e[i])
            shp[:,2:] += np.load(fileshp_pop[i])
        shp[:,0] = np.arange(1,NSW+1)
        shp[:,1:] /= len(fileshp_e)
    ...
    if (l_read_php):
        ...
        php = np.zeros((nmodes,NSW,nqs+2),dtype=np.float32)
        for i in range(len(filephp_e)):
            php[:,:,1] += np.load(filephp_e[i]).T
            php[:,:,2:] += np.swapaxes(np.load(filephp_pop[i]),0,1)
        php[:,:,0] = np.arange(1,NSW+1)
        php[:,:,1:] *= lphinv/len(filephp_e)
    ...
    if (4 in which_plt):
        plot_tdband_sns(k_loc, en, kp_loc, kplabels, shp, k_index, times,
                        Enk=Enk, Eref=Eref, nsw=NSW, nbasis=nbasis, 
                        figname='TDBAND.png')
...
#        nrow = math.ceil( nts / ncol )
    ...
    for it in range(nts):
        ...
        ax.set_title('%.0f fs'%(tindex[it]+1))
...
#        nrow = math.ceil( nts / ncol )
    ...
    for it in range(nts):
        ...
        if (it==0):
            ax.set_title('%.0f fs'%(times[it]+1))
        else:
            ax.set_title('%.0f - %.0f fs'%(times[it-1]+1, times[it]+1))
